...
export default function CreateTicketPanel({ onCreated, refreshToken }) {
    ...
    useEffect(() => {
        (async () => {
            try {
                ...
                const pickArray = (d, key) => Array.isArray(d) ? d : (Array.isArray(d?.[key]) ? d[key] : []);
                setPersons(p.data?.persons ?? pickArray(p.data, "persons") ?? []);
                setEvents(e.data?.events ?? pickArray(e.data, "events") ?? []);
                setVenues(ve.data?.venues ?? pickArray(ve.data, "venues") ?? []);
            } catch (err) {
    ...
    const validate = () => {
        ...
        { const n = asNum(v.price);  if (!pos(n)) err.price = "Число > 0"; }
        if (!v.type) err.type = "Выберите тип";
        { const n = asNum(v.number); if (!Number.isInteger(n) || !(n > 0)) err.number = "Целое > 0"; }
        if (v.discount !== "") {
            const d = asNum(v.discount);
            if (!Number.isFinite(d) || !(d > 0 && d <= 100)) err.discount = "(0;100]";
        }
        { const x = asNum(v.coordX); if (!Number.isFinite(x)) err.coordX = "Число"; }
        { const y = asNum(v.coordY); if (!Number.isFinite(y)) err.coordY = "Число"; }
    ...
    const save = async () => {
        ...
        try {
            ...
            setV({ name:"", price:"", type:"", number:"", discount:"", comment:"", coordX:"", coordY:"", personId:"", eventId:"", venueId:"" });
            setErrors({});
            onCreated?.();
        } catch (e) {
    ...
    return (
        <div className="form-grid" style={{ maxWidth:820 }}>
            ...
            <label>Цена *</label>
            <input className={errors.price ? "err" : ""} type="number" step="0.01" min="0.01" value={v.price} onChange={set("price")} />
            ...
            <input className={errors.number ? "err" : ""} type="number" min="1" value={v.number} onChange={set("number")} />
            {errors.number && <span className="help-err">{errors.number}</span>}

            <label>Скидка (0–100)</label>
            <input className={errors.discount ? "err" : ""} type="number" min="0.01" max="100" step="0.01" value={v.discount} onChange={set("discount")} />
            ...
            <input className={errors.coordX ? "err" : ""} type="number" step="1" value={v.coordX} onChange={set("coordX")} />
            {errors.coordX && <span className="help-err">{errors.coordX}</span>}

            <label>Координата Y *</label>
            <input className={errors.coordY ? "err" : ""} type="number" step="0.01" value={v.coordY} onChange={set("coordY")} />
            {errors.coordY && <span className="help-err">{errors.coordY}</span>}

            <label>Владелец </label>
            <select value={v.personId} onChange={set("personId")}>
                <option value="">— не задан —</option>
                {persons.map(p => <option key={p.id} value={p.id}>{p.passportID || `Person #${p.id}`}</option>)}
            </select>
            ...
            <select value={v.venueId} onChange={set("venueId")}>
                <option value="">— не задано —</option>
                {venues.map(ve => <option key={ve.id} value={ve.id}>{ve.name}</option>)}
            </select>

            <div className="actions" style={{ gridColumn:"1 / -1" }}>
                <button className="btn" onClick={save} disabled={busy}>{busy ? "Сохраняю..." : "Создать Ticket"}</button>
