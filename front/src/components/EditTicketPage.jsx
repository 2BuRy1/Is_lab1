...
const ENDPOINTS = {
    ...
    updateTicket: (id) => `/update_ticket/${id}`,
    persons:           "/get_persons",
    events:            "/get_events",
    venues:            "/get_venues",
};
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                ...
                const personsRaw = pickArray(pRes.data, "persons").map(p => ({
                    ...p,
                    weight: num(p.weight),
                    location: p.location ? {
                        ...p.location,
                ...
                setEvents(pickArray(eRes.data, "events"));
                setVenues(pickArray(vRes.data, "venues"));
                ...
                if (code === 404) {
                    setStatus(`Билет #${id} не найден (404).`);
                } else if (code === 400) {
                    setStatus(`Некорректный запрос (400) ${e.response?.data}`);
                } else {
                    setStatus(`Не удалось загрузить данные: ${code || ""} ${e.message}`);
                }
                setLoadError(true);
            } finally {
        ...
        return (
            <div className="edit-wrap">
                <div className="edit-head">
                    ...
                    <Link className="link-back" to="/">← Назад</Link>
                </div>
        ...
        return (
            <div className="edit-wrap">
                ...
                <div className="actions">
                    <button className="btn" onClick={() => navigate("/")}>К таблице</button>
                </div>
            </div>
        );
    }
            ...
            <div className="edit-card">
                <div className="form-grid">
                    ...
                    >
                        <option value="">— не задан —</option>
                        {persons.map(p => (
                            <option key={p.id} value={p.id}>{p.passportID || `Person #${p.id}`}</option>
                        ))}
                    </select>
