...
export default function CreateTicketPanel({ onCreated, refreshToken }) {
  ...
  const validate = () => {
    ...
    {
      const n = asNum(v.price);
      if (!pos(n)) err.price = 'Число > 0';
    }
    if (!v.type) err.type = 'Выберите тип';
    {
      const n = asNum(v.number);
      if (!Number.isInteger(n) || !(n > 0)) err.number = 'Целое > 0';
    }
    if (v.discount !== '') {
    ...
    {
      const x = asNum(v.coordX);
      if (!Number.isFinite(x)) err.coordX = 'Число';
    }
    {
      const y = asNum(v.coordY);
      if (!Number.isFinite(y)) err.coordY = 'Число';
    }
  ...
  const save = async () => {
    ...
    setBusy(true);
    try {
      await api.post('/add_ticket', buildPayload());
      ...
      setMsg({
        ok: false,
        text: `Ошибка: ${e.response?.status || ''} ${e.response?.data || e.message}`,
      });
    } finally {
      setBusy(false);
    }
